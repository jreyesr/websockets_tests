...
def read_sensor():
    ...
    while True:
        if recording:
            message = "SENSOR,{},{},{},{}\n".format(millis(),int(random.random()*10),int(random.random()*10),int(random.random()*10))
            main_queue.put(message)
            
            # only stream one message in each 4 (AKA once per second)
            # sensor thread is free to stream only what it wants, just call send_entry(message, DATA_STREAMS)
            i = (i + 1) % 4
            if i == 0:
...
def dashboard(path, request_headers):
    if path in ('/', '/dashboard', "/dashboard/"):
        with open('client.html', 'r') as f:
            data = f.read()
            return http.HTTPStatus.OK, [("Content-Type","text/html")], data.encode('ascii')

start_server = websockets.serve(serve, None, 5678, process_request=dashboard)
